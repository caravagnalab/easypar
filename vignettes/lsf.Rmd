...
-   a function `FUN` (e.g.,: `FUN = ls`) that can run as a stand-alone R application, with its own parameters. 

> Note: `FUN` will be run as an indipendent process, so it should manage explicitly  its dependencies.

- a input `dataframe` where every row rappresents an input for the `FUN` (as for `apply` by row). 

The input should have column names without dots or spaces, as these
will be used to match the arguments of `FUN` So for instance, an input with 2-columns will only work if the function is expecting 2 parameters. The fist column will define the first parameter.
...
Before submitting the job, you can test the computation. If the generated script (see below) is called `X` and the input file `Y`, you can test your function with 
...
#### Customisations (optional and required)
...
- specify a list of modules that will be added as dependencies of the LSF submission script. 
  - For instance, `modules = 'R/3.5.0'` will generate the dependecy for a specific R version as `"module load R/3.4.0"` in the script.
